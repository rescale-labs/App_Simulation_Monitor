...
def parse_file(process_output_file_path):
    ...
    header_pattern = r"\s+Iteration\s+Continuity\s+X-momentum\s+Y-momentum.*"
...
def get_df():
    if is_debug():
        # file = find_file("**/starccm_process_output.dat", "..")
        file = find_file("**/starccm_process_output_no_values.dat", "..")
    else:
